...
<section class="commerce-agent">
    ...
    {#if dashboard}
        ...
        {#if dashboard.recovery}<div class="help">Recovery tracking: {dashboard.recovery.pendingReviews} pending reviews · {dashboard.recovery.highRiskPending} high-risk · {dashboard.recovery.appliedChanges} applied · {dashboard.recovery.coverage}% catalog coverage</div>{/if}
    {/if}
            ...
            {#each visible as entry}
                <article class="recommendation-card">
                    ...
                    {#if Object.keys(entry.proposed || {}).length}
                        ...
                        {#if entry.evidence?.length}<div class="help"><b>Evidence:</b> {entry.evidence.map((e) => `${e.field}: ${e.value} (${e.confidence}, ${e.source})`).join(" · ")}</div>{/if}
                        {#if entry.taxonomy}<div class="help"><b>Taxonomy:</b> {entry.taxonomy.message}</div>{/if}
                        {#if entry.soldPricing}<div class="help"><b>Pricing:</b> {entry.soldPricing.message}</div>{/if}
                        {#if entry.demand}<div class="help"><b>Demand proxy:</b> {entry.demand.message}</div>{/if}
                        <div class="actions">
                            <button on:click={() => selected = entry}>Explain Recommendation</button>
                            {#if entry.status === "Pending"}<button class="primary" disabled={loading} on:click={() => approve(entry)}>Approve only</button>{/if}
                            {#if entry.status === "Approved"}<button class="primary" disabled={loading} on:click={() => apply(entry)}>Apply approved change</button>{/if}
                            {#if entry.status === "Applied"}<span class="help">Applied</span>{/if}
                        </div>
