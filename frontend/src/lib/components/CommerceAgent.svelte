<script>
    ...
    async function approve(entry) {
        ...
        error = "";
        try {
            const result = await commerceApprove(
...
<section class="commerce-agent">
    ...
    {#if dashboard}
        <div class="stats commerce-stats">
            <div>
                <strong>{dashboard.listingsFound}</strong><span
                    >Listings found</span
                >
            </div>
            <div>
                <strong>{dashboard.needOptimization}</strong><span
                    >Need optimization</span
                >
            </div>
            <div>
                <strong>{dashboard.titleImprovements}</strong><span
                    >Title opportunities</span
                >
            </div>
            <div>
                <strong>{dashboard.missingItemSpecifics}</strong><span
                    >Missing specifics</span
                >
            </div>
        </div>
    ...
    <div class="commerce-grid">
        <div class="panel nested-panel">
            ...
            {#if !visible.length}<p class="empty">
                    Import your existing eBay listings to create the first audit
                    queue.
                </p>{/if}
                ...
                <article class="recommendation-card">
                    <div class="recommendation-title">
                        <div>
                            <strong
                                >{entry.listing.title ||
                                    "Untitled listing"}</strong
                            ><span
                                >{entry.classification} · Score {entry.score}/100
                                · {entry.status}</span
                            >
                        </div>
                    ...
                    <div class="listing-meta">
                        ...
                        ><span
                            >Listing: {entry.listing.listingId || "none"}</span
                    ...
                    {#if Object.keys(entry.proposed || {}).length}
                        <div class="change-list">
                            {#each Object.entries(entry.proposed) as change}<div
                                >
                                    <b>{change[0]}</b><span
                                        >{String(
                                            entry.listing[change[0]] ||
                                                "Not set",
                                        )} → {String(change[1])}</span
                                    >
                                </div>{/each}
                        </div>
        ...
        {#if selected}
            <aside class="panel nested-panel explanation">
                ...
                <p>
                    <b>Confidence:</b>
                    {selected.confidence}. <b>Risk:</b>
                    {selected.risk}.
                </p>
                <ul>
                    {#each selected.findings as finding}<li>
                            <b>{finding.field}:</b>
                            {finding.message}
                        </li>{/each}
                </ul>
    ...
    <div class="panel nested-panel history-panel">
        ...
        {#each history.slice(0, 10) as entry}<div class="history-row">
                <strong>{entry.listing.title || entry.listing.sku}</strong><span
