...
<div class="min-h-screen flex flex-col">
    <header class="bg-white border-b">
        ...
        >
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 bg-sky-600 rounded flex items-center justify-center text-white font-bold"
                >
                    H
                </div>
                <div>
                    <h1 class="text-lg font-semibold">
                        HHT Catalog — AI Dashboard
                    </h1>
                    <p class="text-xs text-slate-500">
                        Upload an image to analyze SKU, pricing, SEO and vision
                        outputs
                    </p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-4">
                <FileUpload on:fileselected={handleFileSelected}>
                    <div slot="meta" class="text-sm text-slate-600">
                        {fileName}
                    </div>
                </FileUpload>
    ...
    <footer class="text-center text-sm text-slate-400 py-6">
        © {new Date().getFullYear()} HHT Catalog
    </footer>
</div>
